...
function stocks4(ns, data) {
	//ns.tprint(data);
	//ns.tprint(stocksx(ns, data));
	return stocksx(ns, data);
}

function stocksx (ns, data) {
	//ns.tail();
	//ns.print(data);
	...
	for (let i = 0; i < transactions.length; i++) {
		for (let j = i + 1; j < transactions.length; j++) {
			if (transactions[i] < transactions[j]) {
				best = Math.max(best, 0 - transactions[i] + transactions[j], 0 - transactions[i] + transactions[j] + stocksx(ns, [left - 1, transactions.slice(j + 1)]));
			}
		}
	}
	return best;
